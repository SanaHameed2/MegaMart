...
export default function ProductListing() {
  ...
  const { data: categoryData, loading: categoryLoading } = useQuery(
    GET_CATEGORY_AND_CHILDREN,
    {
      ...
      skip: category === 'all' || isSearchMode,
    }
  );

  const categoryIds = useMemo(() => {
    if (category === 'all' || isSearchMode) return null;

    const categoryNode = categoryData?.categoriesCollection?.edges?.[0]?.node;
    ...
    return ids;
  }, [categoryData, category, isSearchMode]);

  const isCategoryResolved = category === 'all' || isSearchMode || categoryIds !== null;
  const isCategoryMissing =
    category !== 'all' && !isSearchMode && !categoryLoading && categoryIds === null;
  ...
  const displayCategory = isSearchMode
